...
def create_measures_dataset() :
    ...
    save_path = MAIN_PATH+"/data/dataset_custom/dataset_measures/"
    ...
    for file in os.listdir(dataset_path) :
        ...
        for index in range(number_of_profiles) :
            ...
            if len(dataset_FULL[0]) > 0 :
                ...
                dataset_FULL = information(dataset_FULL)
                dataset_FULL = pad(dataset_FULL)
                filename = file[:-3]+'_'+str(index)+'.npy'
                numpy.save(save_path+alarm_text[alarm]+label_text[label]+filename, dataset_FULL)

if __name__ == "__main__" :
    create_measures_dataset()