...
def main() :
    ...
    SAVE_PATH = MAIN_PATH+"/data/rfc_random_forest_classifier"
    ...
    criterion = cfg["rfc_random_forest_classifier"]["architecture"]["criterion"]
    max_depth = cfg["rfc_random_forest_classifier"]["architecture"]["max_depth"]
    min_samples_split = cfg["rfc_random_forest_classifier"]["architecture"]["min_samples_split"]
    min_samples_leaf = cfg["rfc_random_forest_classifier"]["architecture"]["min_samples_leaf"]
    max_features = cfg["rfc_random_forest_classifier"]["architecture"]["max_features"]
    max_leaf_nodes = cfg["rfc_random_forest_classifier"]["architecture"]["max_leaf_nodes"]
    ...
    for n in range(50) :
        ...
        rfc = ensemble.RandomForestClassifier(n_estimators=number_of_trees,criterion=criterion, max_depth=max_depth, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, max_features=max_features, max_leaf_nodes=max_leaf_nodes, min_impurity_decrease=min_impurity_decrease) 
        rfc.fit(TrainSet, TrainLabel)
        TestPrediction = rfc.predict(TestSet)
        ...
        cursor = conn.cursor()
        cursor.execute("""SELECT id FROM all_run WHERE M_type LIKE 'RFC' ORDER BY id DESC""")
        tab = cursor.fetchall()
        if len(tab) > 0 :
            id = tab[0][0] + 1
        else :
        ...
        cursor.execute("""INSERT INTO all_run VALUES (%i, 'RFC', '%s', %i, %i, %i)"""%(id, ALARM_TYPE, RANDOM_SEED, T_mult, F_mult))
        conn.commit()
        ...
        for i, x in enumerate(TestInfo) :
            if TestPrediction[i] == TestLabel[i] :
                if x in classification.keys() :
                    classification[x] = (classification[x][0] + 1, classification[x][1] + 1)
                else :
                    classification[x] = (1, 1)
            else :
                if x in classification.keys() :
                    classification[x] = (classification[x][0], classification[x][1] + 1)
                else :
                    classification[x] = (0, 1)

    file = open(SAVE_PATH+"/classification.txt", "w")
    for key in classification.keys() :
        file.write("%s :\t%d\t%d\t%.2f\n"%(key, classification[key][0], classification[key][1], 100*classification[key][0]/classification[key][1]))
    file.close()
